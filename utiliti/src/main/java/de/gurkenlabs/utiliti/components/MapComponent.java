...
public class MapComponent extends GuiComponent {
  ...
  public void loadMaps(Path projectPath) {
    ...
    final List<Path> files = FileUtilities.findFiles(projectPath, ".tmx");
    log.log(Level.INFO, "{0} maps found in folder {1}", new Object[] {files.size(), projectPath});
    final List<TmxMap> loadedMaps = new ArrayList<>();

    files.forEach(f -> {
      TmxMap map = (TmxMap) Resources.maps().get(f.toString());
      if (map != null) {
        loadedMaps.add(map);
      }
      log.log(Level.INFO, "map found: {0}", new Object[] {f.toString()});
    });
